...
class EventBus {
    ...
    emit(eventType, data) {
        ...
        const silentEvents = [
            ...
            // Garder seulement les événements vraiment internes ou de debug
            'PHYSICS_BEFORE_UPDATE', 
            'PHYSICS_AFTER_UPDATE',
            // Événements d'initialisation (OK d'être silencieux)
            'PHYSICS_ENGINE_INITIALIZED',
            'PHYSICS_ENGINE_STARTED',
            'PHYSICS_ENGINE_STOPPED',
            'PHYSICS_ENGINE_RESET',
            'PHYSICS_ENGINE_DESTROYED',
            // Événements d'ajout de corps (informatifs, pas critiques)
            'PHYSICS_BODY_ADDED',
            'PHYSICS_CELESTIAL_BODIES_INITIALIZED',
            // Événements de changement d'état de jeu (si pas d'UI active)
            'GAME_STATE_CHANGED'
            // SUPPRIMÉ : Les événements suivants DOIVENT avoir des listeners !
            // 'PHYSICS_ENGINE_UPDATED', → GameController/RenderingController doivent l'écouter
            // 'PHYSICS_BODY_MOVED', → Système de rendu doit l'écouter  
            // 'PHYSICS_VELOCITY_CHANGED', → Effets visuels doivent l'écouter
            // 'PHYSICS_FORCE_APPLIED', → Système de particules doit l'écouter
            // 'SIMULATION_UPDATED' → RenderingController doit l'écouter
        ];
    ...
    /**
     * Alias pour {@link EventBus#emit}. Publie un événement.
     * @param {string} eventType - Le nom du type d'événement à publier.
     * @param {*} [data] - Données optionnelles à passer aux auditeurs.
     */
