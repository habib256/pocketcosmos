...
class UIView {
    ...
    constructor(eventBus) {
        ...
        this.colors = {
            ...
            ai: '#8cf', // Couleur pour l'indicateur IA
        };
    ...
    subscribeToEvents() {
        ...
        // S'abonner aux événements UI pour changer l'état d'affichage
        ...
        if (EVENTS.UI.HIDE_GAME_OVER_SCREEN) {
        ...
        // Écouter les changements d'état de l'IA
        if (window.EVENTS && window.EVENTS.AI && window.EVENTS.AI.CONTROL_CHANGED) {
            this.eventBus.subscribe(window.EVENTS.AI.CONTROL_CHANGED, (data) => {
                this.aiControlActive = data && data.active;
                console.log("[UIView] État de contrôle IA:", this.aiControlActive ? "Activé" : "Désactivé");
            });
        }
    }
    ...
    _renderRocketInfo(ctx, rocketModel, totalAcceleration) {
        ...
        // --- État de l'IA ---
        const aiY = 110;
        ctx.fillText(`🤖:`, labelX, aiY);
        ctx.fillStyle = this.aiControlActive ? this.colors.ai : this.colors.white;
        ctx.fillText(this.aiControlActive ? 'IA ACTIVE' : 'Manuel', barX, aiY);
        ctx.fillStyle = this.colors.white; // Remettre la couleur par défaut
