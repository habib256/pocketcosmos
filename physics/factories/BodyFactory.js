...
class BodyFactory {
    ...
    constructor(Bodies, Body, Attractors, ROCKET, PHYSICS) {
        ...
        // Créer une fonction d'attraction personnalisée qui utilise notre constante G
        this.customGravity = (bodyA, bodyB) => {
            // Ne pas appliquer de force si l'un des corps est détruit ou inexistant
            if (!bodyA || !bodyB || !bodyA.position || !bodyB.position) {
                return;
            }
            
            // Calculer la distance entre les centres des corps
            const dx = bodyB.position.x - bodyA.position.x;
            const dy = bodyB.position.y - bodyA.position.y;
            const distance = Math.sqrt(dx * dx + dy * dy);
            
            // Éviter la division par zéro et les forces infinies
            if (distance < 1) {
                return;
            }
            
            // Calculer la force gravitationnelle : F = G * m1 * m2 / r²
            const force = this.PHYSICS.G * bodyA.mass * bodyB.mass / (distance * distance);
            
            // Calculer les composantes de la force normalisées
            const forceX = (dx / distance) * force;
            const forceY = (dy / distance) * force;
            
            // Appliquer la force seulement aux corps non-statiques
            if (!bodyA.isStatic) {
                this.Body.applyForce(bodyA, bodyA.position, { x: forceX, y: forceY });
            }
            if (!bodyB.isStatic) {
                this.Body.applyForce(bodyB, bodyB.position, { x: -forceX, y: -forceY });
            }
        };
    }
        ...
        const rocketBody = this.Bodies.rectangle(
            ...
            this.ROCKET.HEIGHT,
            {
                ...
                plugin: {
                    ...
                    attractors: [
                        // Utilise la fonction de gravité de matter-attractors (qui a été remplacée 
                        // par notre version personnalisée dans index.html)
