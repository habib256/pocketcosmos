class PhysicsVectors {
    ...
    update(deltaTime) {
        // Calculer l'accélération gravitationnelle totale si possible
        if (this.physicsController.rocketModel && this.physicsController.universeModel) {
            const gravityVector = PhysicsVectors.calculateGravityVector(
                this.physicsController.rocketModel,
                this.physicsController.universeModel,
                window.PHYSICS ? window.PHYSICS.G : 6.67430e-11
            );
            if (gravityVector) {
                this.setTotalAcceleration(gravityVector.x, gravityVector.y);
            }
        }
    }
