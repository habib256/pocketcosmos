...
class CollisionHandler {
    ...
    constructor(physicsController, engine, Events, Body, ROCKET, PHYSICS) {
        ...
        // Attendre que window.EVENTS soit disponible
        this.setupMissionCompletedListener();
    }

    setupMissionCompletedListener() {
        // Vérifier périodiquement si window.EVENTS est disponible
        const checkEvents = () => {
            if (this.physicsController.eventBus && window.EVENTS && window.EVENTS.MISSION && window.EVENTS.MISSION.COMPLETED) {
                ...
                console.log('[CollisionHandler] Abonné à MISSION.COMPLETED avec succès');
            } else {
