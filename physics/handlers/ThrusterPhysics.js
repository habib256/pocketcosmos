class ThrusterPhysics {
    constructor(physicsController, Body, ROCKET, PHYSICS) {
        ...
        // Vérifier que ASSISTED_CONTROLS existe dans PHYSICS
        if (this.PHYSICS && this.PHYSICS.ASSISTED_CONTROLS) {
            ...
            // Valeurs par défaut si les constantes ne sont pas disponibles
            console.warn('[ThrusterPhysics] ASSISTED_CONTROLS non trouvé dans PHYSICS, utilisation des valeurs par défaut');
            this.angularDamping = 0.0;
            this.assistedAngularDamping = 2.0;
            this.rotationStabilityFactor = 0.05;
        }
    ...
    // Méthode update appelée par PhysicsController
    update(deltaTime) {
        // Récupérer le modèle de la fusée depuis le contrôleur physique
        const rocketModel = this.physicsController.rocketModel;
        if (!rocketModel) return;

        // Mettre à jour les propulseurs
        this.updateThrusters(rocketModel);
        
        // Appliquer la stabilisation de rotation si nécessaire
        this.applyRotationStabilization(rocketModel);
    }
    ...
    handleLiftoff(rocketModel, rocketBody) {
        console.log(`[ThrusterPhysics] *** HANDLELIFTOFF APPELÉE *** depuis ${rocketModel.landedOn || 'surface inconnue'} avec propulseur principal`);
        console.log(`[ThrusterPhysics] État avant handleLiftoff - isLanded: ${rocketModel.isLanded}, landedOn: ${rocketModel.landedOn}`);
        console.log(`[ThrusterPhysics] Position physique avant: x=${rocketBody.position.x.toFixed(1)}, y=${rocketBody.position.y.toFixed(1)}`);
        console.log(`[ThrusterPhysics] Vitesse physique avant: vx=${rocketBody.velocity.x.toFixed(1)}, vy=${rocketBody.velocity.y.toFixed(1)}`);
        ...
        console.log(`[ThrusterPhysics] État après changement - isLanded: ${rocketModel.isLanded}, landedOn: ${rocketModel.landedOn}`);

        // Appliquer une forte impulsion vers le haut (axe de la fusée) pour simuler le décollage
        // Augmentation significative de la force pour un effet plus marqué et visible
        const impulseForce = 50.0; // Force d'impulsion initiale (augmentée de 5.0 à 50.0)
        ...
        console.log(`[ThrusterPhysics] Impulsion calculée: x=${impulse.x.toFixed(2)}, y=${impulse.y.toFixed(2)}, angle=${liftOffAngle.toFixed(2)}`);
        ...
        const initialVelMagnitude = 20.0; // Vitesse initiale ajoutée (augmentée de 2.0 à 20.0)
        const newVelocity = {
        ...
        console.log(`[ThrusterPhysics] Nouvelle vitesse: vx=${newVelocity.x.toFixed(2)}, vy=${newVelocity.y.toFixed(2)}`);
        
        this.Body.setVelocity(rocketBody, newVelocity);
        
        console.log(`[ThrusterPhysics] *** HANDLELIFTOFF TERMINÉE ***`);
    }
